...
static std::string CodeToStaus(int code)//状态码
{
  switch(code)
  {
    case 200:
...
class HttpRequest
{
  ...
  //将请求报文设置成map，形成映射关系
    ...
    bool isNeedRecvBody()
    {
      //post有可能大小写,strcasecmp忽略大小写进行比较
    ...
    //Get方法解析 -> 参数(如果有)在uri之中
    //get方法，路径 ->path，参数(如果有)->query_string
    void GetUriParse()
    {
      std::size_t pos=uri.find('?');
      if(pos==std::string::npos)//没有问号，即没有参数,默认返回一张网页
      {
        path+=uri;
      }
      else
      {
        cgi=true;//有参数，默认使用参数进行计算 -> 使用cgi
        path+=uri.substr(0,pos);//路径
        query_string=uri.substr(pos+1);//参数
      }
    }

    //POST方法解析 -> 资源路径就是uri，参数在正文之中
    void PostUriParse()
    {
      path+=uri;
    }
    ...
    void IsAddHomePage()
    {
      ...
      // 如果以/结尾，只能找到web根目录而找不到资源，默认返回一个网页资源
      if(path.back()=='/')
        path+=HOMEPAGE;
    }
...
class HttpResponse
{
  ...
  public:
    ...
    void SetResponseLine(const std::string &line)//设置响应首行
    {
      response_line=line;
    }
    ...
    void AddHeader(const std::string &ct)//设置响应报文头
    {
      responser_header.push_back(ct);
    }

    const std::vector<std::string> GetResponseHeader()//获取响应报头
    {
 ...
 //-----------------------分析请求、制作响应-----------------
     ...
     void ExceCgi()
     {
      ...
      //站在被调用进程的角度
       ...
       std::string query_env;//定义环境变量(将参数放入其中)
       std::string body;//正文
       ...
       pipe(pipe_out);
        ...
        if(id == 0)//child&&进程程序替换
        {
         ...
         dup2(pipe_out[1],1);
          ...
          else if(req.IsPost())//post方法则需要设置长度 -> 通过管道读取多少字节(参数在正文之中)
          {
            content_length_env = "CONTENT-LENGTH=";
            content_length_env+=std::to_string(req.GetContentLength());
            putenv((char*)content_length_env.c_str());//将正文长度导入环境变量
          }
          else
          {
            //其它方法()
  ...
  public:
   static void HandlerHttp(int sock)
   {
    //命令行式宏定义 -> 用于调试
     ...
     EndPoint *ep=new EndPoint(sock);//创建一个通信模块指针
     ep->RecvRequest(); //处理请求
